...
import { createRefreshQueue } from '~/composables/useRefreshQueue';
...
function setDarkTheme(enabled: boolean) { applyTheme(enabled ? 'dark' : 'light'); }
...
const regionRefreshQueue = createRefreshQueue(() => refreshRegions());
function refreshRegionList() { return regionRefreshQueue.request(); }

let storageEvents: EventSource | undefined;
let eventsMounted = false;

function connectStorageEvents() {
  if (!eventsMounted || storageEvents || !player.value) return;
  storageEvents = new EventSource('/api/events');
  storageEvents.addEventListener('storage:update', () => { void refreshRegionList(); });
}
...
watch(player, connectStorageEvents);
